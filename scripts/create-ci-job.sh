...
# Job name (double-l as requested)
JOB_NAME="weasley-clock-builld"

# Git repository containing the Jenkinsfile
REPO_URL="https://github.com/aviaddia/weasley-clock.git"
GIT_BRANCH="main"
JENKINSFILE_PATH="Jenkinsfile"

# Allow HTTP for lab environment
ALLOW_INSECURE_HTTP="${ALLOW_INSECURE_HTTP:-true}"

# Update if job already exists
UPDATE_EXISTING="${UPDATE_EXISTING:-true}"

# Start a build after creating
...
<flow-definition>
  <actions/>
  <description>CI pipeline for Weasley Clock. Pulls from main, lints, tests, builds Docker images (backend+frontend), pushes to local registry, triggers CD.</description>
...
printf 'Pipeline loads Jenkinsfile from: %s (branch: %s)\n' "$REPO_URL" "$GIT_BRANCH"

if [[ "$RUN_BUILD" == "true" ]]; then
    printf 'Starting first build...\n'
    jenkins_cli build "$JOB_NAME" -s -v
else
    printf 'Job created. Trigger manually or via GitHub webhook.\n'
fi
